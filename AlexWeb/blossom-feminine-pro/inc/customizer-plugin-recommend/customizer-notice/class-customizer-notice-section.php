<?php
/**
 * Blossom Feminine Pro Customizer Notification Section Class.
 *
 * @package Blossom Feminine Pro
 */

/**
 * Blossom_Feminine_Pro_Customizer_Notice_Section class
 */
class Blossom_Feminine_Pro_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		$show_recommended_plugins = get_option( 'blossom_feminine_proshow_recommended_plugins' );

		foreach ( $blossom_feminine_pro_recommended_plugins as $slug => $plugin_opt ) {
			...
			$blossom_feminine_pro_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$blossom_feminine_pro_recommended_plugin['class'] = 'active';
			} else {
				$blossom_feminine_pro_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$blossom_feminine_pro_recommended_plugin['button_class'] = 'install-now button';
					$blossom_feminine_pro_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$blossom_feminine_pro_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$blossom_feminine_pro_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$blossom_feminine_pro_recommended_plugin['button_class'] = 'deactivate-now button';
					$blossom_feminine_pro_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$blossom_feminine_pro_recommended_plugin['id'] = $slug;
			$blossom_feminine_pro_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$blossom_feminine_pro_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$blossom_feminine_pro_recommended_plugin['description'] = $info->short_description;
			}

			$blossom_feminine_pro_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $blossom_feminine_pro_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="blossom-feminine-pro-recommeded-actions-container blossom-feminine-pro-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="blossom-feminine-pro-recommeded-actions">
