<?php
/**
 * Blossom Spa Pro Social Sharing
 * 
...
if( ! function_exists( 'blossom_spa_pro_opengraph_add_prefix' ) ) :
/**
 * Add Open Graph XML prefix to <html> element.
 *
 * @uses apply_filters calls 'opengraph_prefixes' filter on RDFa prefix array
 */
function blossom_spa_pro_opengraph_add_prefix( $output ) {
	$prefixes = array(
		'og' => 'http://ogp.me/ns#',
	);
	$prefixes = apply_filters( 'blossom_spa_pro_opengraph_prefixes', $prefixes );
...
add_filter( 'language_attributes', 'blossom_spa_pro_opengraph_add_prefix' );

/**
 * Register filters for default Open Graph metadata.
 */
function blossom_spa_pro_opengraph_default_metadata() {	
    if( get_theme_mod( 'ed_og_tags', true ) ){
        // additional prefixes
    	add_filter( 'blossom_spa_pro_opengraph_prefixes', 'blossom_spa_pro_opengraph_additional_prefixes' );
        
        // core metadata attributes
    	add_filter( 'blossom_spa_pro_opengraph_title', 'blossom_spa_pro_opengraph_default_title', 5 );
    	add_filter( 'blossom_spa_pro_opengraph_type', 'blossom_spa_pro_opengraph_default_type', 5 );
    	add_filter( 'blossom_spa_pro_opengraph_image', 'blossom_spa_pro_opengraph_default_image', 5 );
    	add_filter( 'blossom_spa_pro_opengraph_url', 'blossom_spa_pro_opengraph_default_url', 5 );
    
    	add_filter( 'blossom_spa_pro_opengraph_description', 'blossom_spa_pro_opengraph_default_description', 5 );
    	add_filter( 'blossom_spa_pro_opengraph_locale', 'blossom_spa_pro_opengraph_default_locale', 5 );
    	add_filter( 'blossom_spa_pro_opengraph_site_name', 'blossom_spa_pro_opengraph_default_sitename', 5 );
    
    	// additional profile metadata
    	add_filter( 'blossom_spa_pro_opengraph_metadata', 'blossom_spa_pro_opengraph_profile_metadata' );
    
    	// additional article metadata
    	add_filter( 'blossom_spa_pro_opengraph_metadata', 'blossom_spa_pro_opengraph_article_metadata' );
    
    	// twitter card metadata
    	add_filter( 'blossom_spa_pro_twitter_card', 'blossom_spa_pro_twitter_default_card', 5 );
    	add_filter( 'blossom_spa_pro_twitter_creator', 'blossom_spa_pro_twitter_default_creator', 5 );    
    }
}
add_action( 'wp', 'blossom_spa_pro_opengraph_default_metadata' );

if( ! function_exists( 'blossom_spa_pro_opengraph_additional_prefixes' ) ) :
/**
 * Add additional prefix namespaces that are supported by the opengraph plugin.
 */
function blossom_spa_pro_opengraph_additional_prefixes( $prefixes ) {
	if ( is_author() ) {
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_title' ) ) :
/**
 * Default title property, using the page title.
 */
function blossom_spa_pro_opengraph_default_title( $title ) {
	if ( $title ) {
		return $title;
	}
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_type' ) ) :
/**
 * Default type property.
 */
function blossom_spa_pro_opengraph_default_type( $type ) {
	if ( empty( $type ) ) {
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_image' ) ) :
/**
 * Default image property, using the post-thumbnail and any attached images.
 */
function blossom_spa_pro_opengraph_default_image( $image ) {
	if ( $image ) {
		return $image;
	}

	// As of July 2014, Facebook seems to only let you select from the first 3 images
	$max_images = apply_filters( 'blossom_spa_pro_opengraph_max_images', 3 );
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_url' ) ) :
/**
 * Default url property, using the permalink for the page.
 */
function blossom_spa_pro_opengraph_default_url( $url ) {
	if ( empty( $url ) ) {
		if ( is_singular() ) {
			$url = get_permalink();
		} else if ( is_author() ) {
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_description' ) ) :
/**
 * Default description property, using the excerpt or content for posts, or the
 * bloginfo description.
 */
function blossom_spa_pro_opengraph_default_description( $description ) {
	if ( $description ) {
		return $description;
	}
	...
	$description = blossom_spa_pro_opengraph_trim_text( $description );

	return $description;
}
endif;

if( ! function_exists( 'blossom_spa_pro_opengraph_default_locale' ) ) :
/**
 * Default locale property, using the WordPress locale.
 */
function blossom_spa_pro_opengraph_default_locale( $locale ) {
	if ( empty( $locale ) ) {
		$locale = get_locale();
	}
...
if( ! function_exists( 'blossom_spa_pro_opengraph_default_sitename' ) ) :
/**
 * Default site_name property, using the bloginfo name.
 */
function blossom_spa_pro_opengraph_default_sitename( $name ) {
	if ( empty( $name ) ) {
...
if( ! function_exists( 'blossom_spa_pro_opengraph_profile_metadata' ) ) :
/**
 * Include profile metadata for author pages.
 *
 * @link http://ogp.me/#type_profile
 */
function blossom_spa_pro_opengraph_profile_metadata( $metadata ) {
	if ( is_author() ) {
		$id = get_queried_object_id();
...
if( ! function_exists( 'blossom_spa_pro_opengraph_article_metadata' ) ) :
/**
 * Include article metadata for posts and pages.
 *
 * @link http://ogp.me/#type_article
 */
function blossom_spa_pro_opengraph_article_metadata( $metadata ) {
	if ( ! is_singular() ) {
		return $metadata;
	}
...
if( ! function_exists( 'blossom_spa_pro_twitter_default_card' ) ) :
/**
 * Default twitter-card type.
 */
function blossom_spa_pro_twitter_default_card( $card ) {
	if ( $card ) {
		return $card;
	}

	$card = 'summary';
	$images = apply_filters( 'blossom_spa_pro_opengraph_image', null );
...
if( ! function_exists( 'blossom_spa_pro_twitter_default_creator' ) ) :
/**
 * Default twitter-card creator.
 */
function blossom_spa_pro_twitter_default_creator( $creator ) {
	...
	$post = get_queried_object();
	$author = $post->post_author;
	$socials = get_user_meta( $author, '_blossom_spa_pro_user_social_icons', true );
...
if( ! function_exists( 'blossom_spa_pro_opengraph_metadata' ) ) :
/**
 * Get the Open Graph metadata for the current page.
 *
 * @uses apply_filters() Calls 'blossom_spa_pro_opengraph_{$name}' for each property name
 * @uses apply_filters() Calls 'blossom_spa_pro_twitter_{$name}' for each property name
 * @uses apply_filters() Calls 'blossom_spa_pro_opengraph_metadata' before returning metadata array
 */
function blossom_spa_pro_opengraph_metadata() {
	$metadata = array();
	...
	foreach ( $properties as $property ) {
		$filter = 'blossom_spa_pro_opengraph_' . $property;
	...
	foreach ( $twitter_properties as $property ) {
		$filter = 'blossom_spa_pro_twitter_' . $property;
		$metadata[ "twitter:$property" ] = apply_filters( $filter, '' );
	}

	return apply_filters( 'blossom_spa_pro_opengraph_metadata', $metadata );
}
endif;

if( ! function_exists( 'blossom_spa_pro_opengraph_site_icon_image_sizes' ) ) :
/**
 ...
 */
function blossom_spa_pro_opengraph_site_icon_image_sizes( $sizes ) {
	$sizes[] = 512;

	return array_unique( $sizes );
}
endif;
add_filter( 'site_icon_image_sizes', 'blossom_spa_pro_opengraph_site_icon_image_sizes' );

if( ! function_exists( 'blossom_spa_pro_opengraph_trim_text' ) ) :
/**
 * Helper function to trim text using the same default values for length and
 * 'more' text as wp_trim_excerpt.
 */
function blossom_spa_pro_opengraph_trim_text( $text ) {
...
if( ! function_exists( 'blossom_spa_pro_opengraph_meta_tags' ) ) :
/**
 * Output Open Graph <meta> tags in the page header.
 */
function blossom_spa_pro_opengraph_meta_tags() {
	if( get_theme_mod( 'ed_og_tags', true ) ){
        $metadata = blossom_spa_pro_opengraph_metadata();
...
add_action( 'wp_head', 'blossom_spa_pro_opengraph_meta_tags' );

if( ! function_exists( 'blossom_spa_pro_get_social_share' ) ) :
/**
 * Get list of social sharing icons
 * http://www.sharelinkgenerator.com/
 * 
*/
function blossom_spa_pro_get_social_share( $share ){
    global $post;
...
if( ! function_exists( 'blossom_spa_pro_social_share' ) ) :
/**
 * Prints Social Sharing
*/
function blossom_spa_pro_social_share(){ 
		...
		<ul class="social-list">
			<?php
                foreach( $social_share as $share ){
                    blossom_spa_pro_get_social_share( $share );
                }
            ?>
		</ul>
	</div>
